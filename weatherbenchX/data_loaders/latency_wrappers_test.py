...
class LatencyWrappersTest(absltest.TestCase):
  ...
  def test_multiple_latency_wrappers_with_all_init_times_missing_raises_error(
      self,
  ):
    prediction = test_utils.mock_prediction_data(
        time_start='2020-01-01T00',
    ...
    # Query time: 00.
    # Wrapper 1 (6h): issue time 06, returns None.
    # Wrapper 2 (1h): issue time 01, returns None.
    # ValueError should be raised.
    init_times = np.array(['2020-01-01T00'], dtype='datetime64[ns]')
    lead_times = np.array([1], dtype='timedelta64[h]')

    with self.assertRaisesRegex(
        ValueError, 'No available init time found for init time'
    ):
      multi_wrapper.load_chunk(init_times, lead_times)
