...
class DataLoader(abc.ABC):
  ...
  def __init__(
      ...
      add_raw_values_to_coords: bool = False,
  ):
    ...
    Args:
      ...
      add_raw_values_to_coords: If True, add raw values to coordinates. These
        can be used for binning. Default: False.
    """
    self._interpolation = interpolation
    self._compute = compute
    self._add_nan_mask = add_nan_mask
    self._process_chunk_fn = process_chunk_fn
    self._add_raw_values_to_coords = add_raw_values_to_coords
  ...
  def load_chunk(
    ...
    if self._add_raw_values_to_coords:
      chunk = xarray_tree.map_structure(
          lambda da: da.assign_coords(raw_values=da), chunk
      )
