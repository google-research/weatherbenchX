...
class Aggregator:
  ...
  Attributes:
    ...
    modifiers: List of modifier instances, each of which can specify one or
      more weightings or binnings of statistics to compute during aggregation.
      ...
      Modifiers are all applied multiplicatively before reducing over the
      `reduce_dims`, in a single xr.dot. They can add new dimensions (e.g. for
      different bins) and if multiple modifiers do this, we end up with the
      Cartesian product of all the added dimensions.
    bin_by: Deprecated. List of modifiers that perform binning. Please specify
      these as `modifiers` instead.
    weigh_by: Deprecated. List of simple modifiers that don't add new
      dimensions. Please specify these as `modifiers` instead.
  ...
  modifiers: Sequence[modifier.Modifier] = ()
  bin_by: dataclasses.InitVar[Sequence[modifier.Modifier] | None] = None
  weigh_by: dataclasses.InitVar[Sequence[modifier.Modifier] | None] = None
  masked: bool = False
  skipna: bool = False

  def __post_init__(
      self,
      bin_by: Sequence[modifier.Modifier] | None,
      weigh_by: Sequence[modifier.Modifier] | None,
  ):
    modifiers = list(self.modifiers)
    if bin_by is not None:
      warnings.warn(
          'bin_by is deprecated and will be removed in a future version. '
          'Please use modifiers instead.',
          DeprecationWarning,
          stacklevel=2,
      )
      modifiers.extend(bin_by)
