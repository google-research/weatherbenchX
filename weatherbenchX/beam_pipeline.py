...
class LoadPredictionsAndTargets(beam.DoFn):
  ...
  def process(
      self, all_inputs: Tuple[int, Tuple[np.ndarray, Union[np.ndarray, slice]]]
  ) -> Iterable[Tuple[int, Tuple[Mapping[Hashable, xr.DataArray], Mapping[Hashable, xr.DataArray]]]]:
...
class ComputeAndFormatStatistics(beam.DoFn):
  ...
  def process(
    ...
    init_index, lead_index = self.times.get_init_and_lead_index(chunk_index)
...
def define_unaggregated_pipeline(
  ...
  for dim, size in dim_sizes.items():
    if dim == 'init_time':
      stat_chunks[dim] = times.init_time_chunk_size
    elif dim == 'lead_time':
      stat_chunks[dim] = times.lead_time_chunk_size
    else:
