...
class StatisticalInferenceMethod(abc.ABC):
  ...
  * Differences in Metrics between two aligned datasets of predictions/targets.
  ...
  def __init__(
    ...
    self._baseline_aggregated_statistics = baseline_aggregated_statistics
  ...
  def confidence_intervals(
    ...
    These are intervals which are (ideally at least) expected to contain the
    true underlying value with probability 1-alpha under repeated resamplings of
    the dataset from the same underlying distribution.
  ...
  def significance_tests(
      self, null_value: float = 0, alpha: float = 0.05) -> MetricValues:
