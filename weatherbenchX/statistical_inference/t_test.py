...
def _autocorrelation_estimate(
  ...
  # Drop any index coordinate to allow alignment at lagged offsets:
  data_array = data_array.drop_vars(dim, errors='ignore')
  ...
  * Independence of the statistic across experimental units. This is important
  ...
  def __init__(
    ...
    super().__init__(
        ...
        baseline_aggregated_statistics=baseline_aggregated_statistics,
    )
  ...
  def _check_constant_weights(self, weights: xr.DataArray) -> None:
    try:
      xr.testing.assert_allclose(
          *xr.broadcast(
              weights.isel({self._dim: 0}),
              weights,
          ),
      )
    except AssertionError as e:
      raise ValueError(
          'aggregated statistics must have the same sum_weights for all values '
          f'along experimental_unit_dim {self._dim} in order to apply the '
          f't-test, but weights for {weights.name} were not equal.'
      ) from e
