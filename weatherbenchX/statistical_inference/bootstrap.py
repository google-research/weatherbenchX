...
class StationaryBootstrap(Bootstrap):
  ...
  done separately for every component of the metric result.
  ...
  selection, which is more justified the closer to linear the function f is
  over the range of values typical of X.
  ...
  autocorrelation in some cases. The VAR model is also somewhat complicated.

  We may nevertheless try implementing some of these alternative methods in
  future. From what I understand though, automatic block-length selection for
  bootstrap methods applied to multivariate time series data is a difficult open
  problem in statistics. If you don't like the default approach, you are free to
  manually specify the block length to use.
