...
class BootstrapTest(absltest.TestCase):
  ...
  def test_bca_adjust_cdf_values_and_inverse_identity_with_0_bias_accel(self):
    inputs = xr.DataArray([0.0, 1e-6, 0.01, 0.1, 0.5, 0.9, 0.99, 1-1e-6, 1.0])
    results = bootstrap._bca_adjust_cdf_values(inputs, bias=0, acceleration=0)
    np.testing.assert_allclose(results, inputs, atol=1e-6)
    results = bootstrap._bca_inverse_adjust_cdf_values(
        inputs, bias=0, acceleration=0)
    np.testing.assert_allclose(results, inputs, atol=1e-6)

  def test_bca_adjust_cdf_values_and_inverse_roundtrip(self):
    inputs = xr.DataArray([0.0, 1e-6, 0.01, 0.1, 0.5, 0.9, 0.99, 1-1e-6, 1.0])
    bias = 0.1
    acceleration = 0.2

    adjusted = bootstrap._bca_adjust_cdf_values(inputs, bias, acceleration)
    roundtrip = bootstrap._bca_inverse_adjust_cdf_values(
        adjusted, bias, acceleration)
    np.testing.assert_allclose(roundtrip, inputs, atol=1e-6)

    inv_adjusted = bootstrap._bca_inverse_adjust_cdf_values(
        inputs, bias, acceleration)
    roundtrip = bootstrap._bca_adjust_cdf_values(
        inv_adjusted, bias, acceleration)
    np.testing.assert_allclose(roundtrip, inputs, atol=1e-6)
  ...
  def test_iid_bootstrap_inference_of_exp_of_mean(self):
    ...
    for method in ["bca", "bc", "percentile"]:
      for alpha in [0.05, 0.1, 0.2]:
        with self.subTest(f"{method=} {alpha=}"):
...
class StationaryBootstrapTest(absltest.TestCase):
  ...
  def test_leave_block_out_sums(self):
    # pylint: disable=bad-continuation  # As we can't 0-pad int literals.
    sums = bootstrap._leave_block_out_sums(
        data=np.array([
                 1,
                10,
               100,
        ...
        block_lengths=np.array([1, 2, 2, 1, 3, 2]),
    )
    np.testing.assert_array_equal(
        sums,
        [
             11111,  # Leave out block length 1 at index 0
            100111,  # Leave out block length 2 at index 1
            110011,  # Leave out block length 2 at index 2
            111011,  # Leave out block length 1 at index 3
             11100,  # Leave out block length 3 at index 4 (wraps around)
             11110,  # Leave out block length 2 at index 5 (wraps around)
        ],
    )
