...
class BootstrapTest(absltest.TestCase):
  ...
  def test_iid_bootstrap_inference_of_exp_of_mean(self):
    ...
    for alpha in [0.05, 0.2, 0.1, 0.05]:
...
class StationaryBootstrapTest(absltest.TestCase):
  ...
  def test_stationary_bootstrap_inference_of_mean_of_ar1_process(self):
    ...
    # Check the point estimates center approximately around the true mean.
    ...
    # Now check that the standard error estimates center approximately around
    # the true standard error of the sample mean.
  ...
  def test_nonlinear_function_of_vector_mean(self):
    ...
    # We check that our bootstrap estimates of the standard error center around
    # our (independently-computed and better) estimate of its true value.
