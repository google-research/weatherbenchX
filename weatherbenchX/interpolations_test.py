...
class CropToBoxTest(absltest.TestCase):
  ...
  def test_crop_to_box_wrap_around_init(self):
    # This should raise ValueError because 300 > 60 initially.
  ...
  def test_crop_to_box_invalid_lon(self):
    with self.assertRaisesRegex(ValueError, 'Invalid longitudes.*'):
      interpolations.CropToBox(lat_min=-10, lat_max=10, lon_min=10, lon_max=0)
